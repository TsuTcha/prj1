...
if st.session_state.start_button_clicked:
    ...
    st.markdown("以下のプロフィール情報をもとに、適宜AIの予想を参考にしながら、生徒1の成績を予想してください。", unsafe_allow_html=True)

    st.markdown("<hr>", unsafe_allow_html=True)

    st.markdown("・生徒1のプロフィール", unsafe_allow_html=True)
    ...
    #st.markdown("""
    #        成績の予想ができたら、該当する項目を選んで、最後に提出ボタンを押してください。<br>
    #        """, unsafe_allow_html=True)
    
    #selected_option = st.selectbox("生徒の成績はどちらだと思いますか？", [None, "平均より優れている", "平均よりも劣っている"], format_func=lambda x: "選択してください" if x is None else x)

    #if selected_option is not None:
    #    st.write(f"選択された成績: {selected_option}")

    #st.write("")

    #st.markdown("成績を選んだら、以下の提出ボタンを押してください。", unsafe_allow_html=True)

    # ボタンが押されたらファイルアップデート&次の問題へ
    #if st.button('回答を提出'):
